...
export default withEnglishFallback({
  ...
  connection: {
    ...
    sqliteWorkerPlacementSessionHint: "由 DBX 经 SSH 把 Worker 上传到文件主机，断开连接后自动删除远端文件。适合临时使用场景。",
    sqliteWorkerPlacementPersistHint: "由 DBX 经 SSH 把 Worker 上传到文件主机，断开后仍保留在远端，不会自动删除。",
    sqliteWorkerPlacementPreplacedHint: "需要手动将 Worker 上传到文件主机，再填写文件的绝对路径。DBX 不会自动上传或删除，但会校验文件是否与当前 Desktop 版本一致。",
