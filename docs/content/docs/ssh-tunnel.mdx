...
The first Test or Connect that would upload the worker asks for consent and shows the destination path and digest. You can keep the worker for the session only, persist it under `~/.cache/dbx/sqlite-worker/<digest>`, or point at a pre-placed binary. Desktop installs the **SQLite SSH Worker** from Driver Manager as one package with both Linux x64 and Linux ARM64 binaries, then uploads the one that matches the file host. SQLCipher, loadable extensions, and attached databases are not supported on that remote path in v1. Backup saves a SQLite snapshot to your laptop; restore uploads a local SQLite file and applies it on the file host.
